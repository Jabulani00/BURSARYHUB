...
import { fetchBursaryById, updateBursary, clearCurrentBursary } from '../../store/slices/bursarySlice';
...
type EditBursaryNavigationProp = NativeStackNavigationProp<ProviderStackParamList, 'EditBursary'>;
type EditBursaryRouteProp = RouteProp<ProviderStackParamList, 'EditBursary'>;

const EditBursaryScreen: React.FC = () => {
  const navigation = useNavigation<EditBursaryNavigationProp>();
  const route = useRoute<EditBursaryRouteProp>();
  ...
  useEffect(() => {
    if (currentBursary) {
      setFormData({
        ...
        description: currentBursary.description,
        requirements: currentBursary.requirements.length > 0 ? currentBursary.requirements : [''],
        deadline: currentBursary.deadline.toISOString().split('T')[0],
        fieldOfStudy: currentBursary.fieldOfStudy.length > 0 ? currentBursary.fieldOfStudy : [''],
        level: currentBursary.level,
        amount: currentBursary.amount,
        duration: currentBursary.duration || '',
        location: currentBursary.location || '',
      });
    }
  }, [currentBursary]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await dispatch(updateBursary({ id: bursaryId, ...updateData })).unwrap();
      ...
      Alert.alert('Error', 'Failed to update bursary');
    }
  };
  ...
  if (!currentBursary) {
    return (
      <View style={styles.container}>
        <Header title="Edit Bursary" showBackButton />
...
export default EditBursaryScreen;
